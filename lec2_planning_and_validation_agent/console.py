...
class Console:
    ...
    def _render(self, e: dict) -> None:
        ...
        if kind == "model_call":
            ...
            # The role comes from the event. Hardcoding "controller" was right
            # when there was one model node and is a lie now that there are three.
            self._line(MODEL, "MODEL", f"{e.get('role', 'controller')} → {asked}",
            ...
            by, decision = e.get("by", ""), e["decision"]
            if decision in ("accepted", "rejected"):
                colour = GOOD + Style.BRIGHT if decision == "accepted" else BAD
                self._line(colour, "VERDICT", decision, f"decided by {by}")
                for reason in e.get("reasons") or []:
                    self._line(colour, "", f"  - {reason}")
                return
            colour = MODEL if by == "model" else RUNTIME
            # Where a route carries a reason, the reason is the interesting part;
            # the colour already says who decided.
            self._line(colour, "ROUTE", f"→ {decision}",
                       e.get("reason") or f"decided by {by}")
            ...
            if checks:
                ...
                for step in steps:
                    self._line(MODEL, "", f"  {step['id']}. {step['goal']}")
                    if step.get("evidence"):
                        self._line(NARRATIVE, "", f"     evidence: {step['evidence']}")
            else:
    ...
    def calling(self, message_count: int, role: str = "controller") -> None:
        """A model call has been issued. Printed before the wait, not after.

        The role is named because three different actors now spend model calls,
        and a reader watching the run needs to know which one is thinking.
        """
        self._line(MODEL, "MODEL →", f"{role} thinking over {message_count} messages…")
    ...
    def trajectory(self, events: list[dict], style: str = "mermaid",
        ...
        """
        from lec2_planning_and_validation_agent.trajectory import to_mermaid, to_text
    ...
    def report(self, state: dict, budget: int, trace_path) -> None:
        ...
        revised = " (revised once)" if state.get("replanned_at") else ""
        print(f"plan            : {len(plan)} steps{revised}")
        validation = state.get("validation")
        if validation:
            decided = validation.get("decided_by", "model")
            print(f"validation      : "
                  f"{'accepted' if validation['accepted'] else 'rejected'} by {decided}")
