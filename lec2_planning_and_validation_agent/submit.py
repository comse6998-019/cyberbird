...
from lec2_planning_and_validation_agent.workspace import AgentWorkspace
...
class Checks:
    ...
    def passed(self) -> bool:
        """All three, and the only thing that decides whether `validate` runs."""
        return self.applies and self.touches_line and self.rescan_clean

    def as_dict(self) -> dict:
        # `passed` is a property, and asdict() does not serialise properties.
        # The routing predicate reads this dict back out of state rather than
        # holding the dataclass, so the key is written explicitly. Leave it out
        # and `after_submit` reads None from a dict that looks complete, treats
        # a clean patch as failing, and the run loops back to the controller
